...
public class PrivateMessageHandler extends SimpleChannelInboundHandler<HttpRequest> {

    private Logger logger = Logger.getLogger(PrivateMessageHandler.class);
    private InetSocketAddress address;
    private String host;
    public PrivateMessageHandler(InetSocketAddress address){
    ...
    protected void messageReceived(ChannelHandlerContext ctx, HttpRequest request) throws Exception {
        ...
        String authentication = request.headers().get("authentication").toString();
        if (authentication.equals("passby")){
